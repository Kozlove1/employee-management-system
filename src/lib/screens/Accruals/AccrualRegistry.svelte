<script lang="ts">
	...
	let filteredAccruals = $derived(() => {
		...
		const employeeGuid = selectedEmployee
			? mockEmployees.find((e) => e.employee === selectedEmployee)?.employee_guid
			: undefined

		const typeGuid = selectedType
			? accrualTypesStore.types.find((t) => t.type_name === selectedType)?.type_guid
			: undefined
	...
	async function handleAddAccrual(data: AccrualFormData) {
		...
		AccrualsDataManager.create(createData, mockEmployees, accrualTypesStore.types)
	...
	async function handleUpdateAccrual(data: AccrualFormData) {
		...
		const result = AccrualsDataManager.update(updateData, mockEmployees, accrualTypesStore.types)
