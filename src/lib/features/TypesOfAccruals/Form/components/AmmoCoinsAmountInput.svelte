	...
	<div class="space-y-3">
		...
		{#if hasFixedAmount}
			<input
				...
				min="0"
				step="1"
				class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-500"
				class:border-red-300={error}
				class:focus:ring-red-500={error}
				class:focus:border-red-500={error}
			/>
			<p class="text-xs text-gray-500">
				Если указано, то при создании начисления сумма будет автоматически установлена в это
				значение
			</p>
		{:else}
			<p class="text-xs text-gray-500">
				Если не указано, то при создании начисления сумму можно будет ввести вручную
			</p>
		{/if}
	</div>
</FormField>
