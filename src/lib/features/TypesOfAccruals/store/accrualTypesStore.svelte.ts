...
class AccrualTypesStore {
	...
	async fetchTypes(params?: AccrualTypeSearchParams): Promise<void> {
		...
		try {
			...
			this.setError(err instanceof Error ? err.message : 'Ошибка загрузки типов начислений')
			this._typeOptionsCache = null
			this._typeOptionsCacheKey = ''
		} finally {
	...
	public getTypesWithFixedAmount(): AccrualType[] {
		return this.types.filter((type) => type.ammo_coins_amount && type.ammo_coins_amount > 0)
	}
	...
	async fetchStats(): Promise<void> {
		...
		try {
			const response = await accrualTypesApi.getAll({ page: -1 })
			...
			if (response.status === 'success') {
				...
				const allTypes: AccrualType[] = list.map(
					(type: AccrualTypeFromApi): AccrualType => ({
						...type,
						type_guid: type.id
					})
				...
				const withFixed = allTypes.filter((t) => (t.ammo_coins_amount ?? 0) > 0).length
				const withVariable = allTypes.filter((t) => (t.ammo_coins_amount ?? 0) === 0).length

				this.stats = {
					total_types: totalTypes,
					types_with_fixed_amount: withFixed,
					types_with_variable_amount: withVariable
				}
			} else {
				this.statsError = response.message || 'Ошибка загрузки статистики типов начислений'
				this.stats = null
			}
		} catch (err) {
			this.statsError =
				err instanceof Error ? err.message : 'Ошибка загрузки статистики типов начислений'
			this.stats = null
		} finally {
	...
	async initialize(): Promise<void> {
		...
		await Promise.all([this.fetchTypes(), this.fetchStats()])
	}
}
